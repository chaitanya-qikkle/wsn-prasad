...
export default function SplashPage() {
  ...
  return (
    <Box sx={{ minHeight: '100vh', background: '#f8fafc', color: '#0f172a', position: 'relative', overflow: 'hidden' }}>
      {/* top bar */}
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', px: { xs: 2, md: 6 }, py: 2.2,
        borderBottom: '1px solid rgba(15,23,42,0.08)', position: 'relative', zIndex: 2, background: '#fff' }}>
        <Stack direction="row" spacing={1.4} alignItems="center">
          <Box sx={{ width: 34, height: 34, borderRadius: '9px', background: ACCENT,
            display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <SensorsIcon sx={{ fontSize: 18, color: '#fff' }} />
          </Box>
          <Typography fontWeight={700} sx={{ color: '#0f172a', letterSpacing: 1 }}>TRUSTCHAIN-WSN</Typography>
        </Stack>
        <Stack direction="row" spacing={2} alignItems="center">
          ...
          <Button variant="outlined" size="small" onClick={() => navigate('/login')}
            sx={{ borderColor: `${ACCENT}55`, color: ACCENT, '&:hover': { borderColor: ACCENT, background: `${ACCENT}0d` } }}>
            Sign In
          </Button>
        </Stack>
      </Box>
      ...
      <Grid container sx={{ position: 'relative', zIndex: 1, minHeight: 'calc(100vh - 69px)' }} alignItems="center">
        ...
        <Grid item xs={12} md={6.5} sx={{ px: { xs: 3, md: 8 }, py: { xs: 5, md: 0 } }}>
          <Stack spacing={2.5} sx={{ animation: `${fadeUp} 0.7s ease both` }}>
            <Stack direction="row" spacing={1.5} alignItems="center">
              ...
              <Box>
                <Typography fontSize="0.78rem" fontWeight={700} color="#0f172a">Pillai HOC College of Engineering and Technology</Typography>
                <Typography fontSize="0.6rem" letterSpacing={2} color="rgba(15,23,42,0.45)">DEPARTMENT OF MCA · 2025–26</Typography>
              </Box>
            </Stack>

            <Chip label="FINAL YEAR PROJECT · MCA" size="small"
              icon={<VerifiedUserIcon sx={{ fontSize: '14px !important', color: `${ACCENT} !important` }} />}
              sx={{ alignSelf: 'flex-start', background: `${ACCENT}0f`, border: `1px solid ${ACCENT}33`, color: ACCENT, fontWeight: 700, fontSize: '0.6rem', letterSpacing: 1 }} />

            <Typography variant="h2" fontWeight={700} sx={{ fontSize: { xs: '2.2rem', md: '3.1rem' }, lineHeight: 1.1, color: '#0f172a' }}>
              Secure Routing,<br />
            ...
            <Typography sx={{ color: 'rgba(15,23,42,0.6)', maxWidth: 520, fontSize: '0.95rem', lineHeight: 1.7 }}>
            ...
            <Stack direction="row" spacing={1.2} flexWrap="wrap" useFlexGap>
              {[
                ...
                { icon: <VerifiedUserIcon sx={{ fontSize: 14 }} />, label: 'Trust-Aware AODV', c: '#7c3aed' },
                ...
                <Stack key={p.label} direction="row" spacing={0.7} alignItems="center"
                  sx={{ px: 1.6, py: 0.7, borderRadius: 5, background: '#fff', border: '1px solid rgba(15,23,42,0.08)', color: p.c }}>
                  {p.icon}<Typography fontSize="0.72rem" fontWeight={600} color="rgba(15,23,42,0.65)">{p.label}</Typography>
                </Stack>
              ))}
            </Stack>
            ...
            <Stack direction="row" spacing={2} pt={1}>
              <Button variant="contained" size="large" endIcon={<ArrowForwardIcon />} onClick={() => navigate('/login')}
                sx={{ background: ACCENT, color: '#fff', fontWeight: 700, letterSpacing: 0.4, px: 4, py: 1.3,
                  borderRadius: 1.5, '&:hover': { background: '#1d4ed8' } }}>
                Enter Control Room
              </Button>
              <Button variant="outlined" size="large" onClick={() => navigate('/register')}
                sx={{ borderColor: 'rgba(15,23,42,0.16)', color: 'rgba(15,23,42,0.7)', px: 4, py: 1.3, borderRadius: 1.5,
                  '&:hover': { borderColor: ACCENT, color: ACCENT, background: `${ACCENT}0a` } }}>
                Create Account
              </Button>
            </Stack>
            ...
            <Stack direction="row" spacing={4} pt={2}>
              <Box>
                <Typography fontSize="0.58rem" letterSpacing={2} color={ACCENT}>SUBMITTED BY</Typography>
                <Typography fontWeight={700} fontSize="0.85rem" color="#0f172a">Prasad Kathare · Pradnya Desai</Typography>
              </Box>
              <Box sx={{ borderLeft: '1px solid rgba(15,23,42,0.1)', pl: 4 }}>
                <Typography fontSize="0.58rem" letterSpacing={2} color={ACCENT}>GUIDE</Typography>
                <Typography fontWeight={700} fontSize="0.85rem" color="#0f172a">Prof. Jitesh Mhatre</Typography>
              </Box>
              <Box sx={{ borderLeft: '1px solid rgba(15,23,42,0.1)', pl: 4 }}>
                <Typography fontSize="0.58rem" letterSpacing={2} color={ACCENT}>HEAD OF DEPT.</Typography>
                <Typography fontWeight={700} fontSize="0.85rem" color="#0f172a">Prof. Abhijeet More</Typography>
              </Box>
            </Stack>
          </Stack>
        </Grid>
        ...
        <Grid item xs={12} md={5.5} sx={{ display: { xs: 'none', md: 'flex' }, justifyContent: 'center', alignItems: 'center', position: 'relative' }}>
          <Box sx={{ width: 440, height: 440, position: 'relative' }}>
            <Box sx={{ position: 'absolute', inset: 0, borderRadius: '50%', border: `1px solid ${ACCENT}22` }} />
            <Box component="svg" viewBox="0 0 100 100" sx={{ width: '100%', height: '100%' }}>
              {GRAPH_EDGES.map(([a, b], i) => {
                ...
                return <line key={i} x1={nm[a].x} y1={nm[a].y} x2={nm[b].x} y2={nm[b].y}
                  stroke={bad ? '#dc2626' : ACCENT2} strokeOpacity={bad ? 0.7 : 0.5} strokeWidth={0.6}
              ...
              {GRAPH_NODES.map(n => (
                <g key={n.id}>
                  ...
                  <circle cx={n.x} cy={n.y} r={n.r / 2.2}
                    fill={n.bad ? '#dc2626' : n.sink ? ACCENT : ACCENT2}
                    opacity={0.9}>
            ...
            <Box sx={{ position: 'absolute', bottom: 8, right: 8, px: 1.4, py: 0.6, borderRadius: 2,
              ...
              <Typography fontSize="0.6rem" color="rgba(15,23,42,0.55)" fontFamily="monospace">
                <span style={{ color: '#dc2626' }}>● </span>1 malicious isolated · chain sealed
              </Typography>
            </Box>
          </Box>
        </Grid>
      ...
      <Box sx={{ borderTop: '1px solid rgba(15,23,42,0.06)', py: 2.2, textAlign: 'center', position: 'relative', zIndex: 1, background: '#fff' }}>
        <Typography variant="caption" color="rgba(15,23,42,0.4)">
